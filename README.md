# Hi, I'm Snigdha Raghavan

Aspiring Data Analyst | AI, RAG & Cloud Projects  
Building data-driven and intelligent systems

---

## Tech Stack

### Data & Analytics
![Python](https://img.shields.io/badge/Python-3.11-blue)
![Pandas](https://img.shields.io/badge/Pandas-Data%20Analysis-lightgrey)
![NumPy](https://img.shields.io/badge/NumPy-Numerical%20Computing-lightgrey)
![Excel](https://img.shields.io/badge/Excel-Advanced-green)
![Power BI](https://img.shields.io/badge/PowerBI-Visualization-yellow)

### AI / ML
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-Models-orange)
![NLP](https://img.shields.io/badge/NLP-Text%20Processing-orange)
![RAG](https://img.shields.io/badge/RAG-LangChain-blue)

### Cloud & Tools
![GCP](https://img.shields.io/badge/GCP-Cloud-blue)
![FastAPI](https://img.shields.io/badge/FastAPI-Backend-green)
![Streamlit](https://img.shields.io/badge/Streamlit-App-red)
![Git](https://img.shields.io/badge/Git-Version%20Control-black)

---

## Projects

### RAG Chatbot
Context-aware PDF chatbot using LangChain and Groq with semantic search and structured responses.

### Resume Analyzer
AI-based resume evaluation system with skill gap analysis, scoring, and insights.

### Credit Card Fraud Detection
Machine learning model for detecting fraudulent transactions on imbalanced datasets with optimized precision-recall.

### Data Visualization Dashboard
Interactive D3.js dashboard analyzing behavioral and performance patterns.

---

## GitHub Stats
![Stats](https://github-readme-stats-sigma-five.vercel.app/api?username=snigdha031&show_icons=true)

---

## Contact
LinkedIn: https://www.linkedin.com/in/snigdha-rp-32a03820a/
